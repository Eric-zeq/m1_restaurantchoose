...
class WhosGoingScreen extends React.Component {
    ...
    async componentDidMount() {
      ...
      this.setState({people: parsedPeople, selected: selected});
    } catch (error) {
      ...
      try{
        ...
        const filteredRestaurants = parsedRestaurants.filter((restaurant) => {
          return (
            ...
            (price === "" || restaurant.price <= parseInt(price)) &&
        ...
        if (filteredRestaurants.length === 0) {
          Alert.alert (
            ...
            "None of your restaurants match your criteria. Maybe try loosening them up a bit?",
            [{text: "OK"}],
            {cancelable: false}
          );
        } else {
